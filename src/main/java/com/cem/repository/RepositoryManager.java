...
public class RepositoryManager<T,ID> implements ICrud<T,ID>{
    ...
    public List<Hasta> findByName(String hastaAdi){
        openSession();
        List<Hasta> hastaList = new ArrayList<>();
         hastaList = em.createQuery("SELECT h FROM Hasta h WHERE h.hastaAdi = :hastaAdi", Hasta.class)
                .setParameter("hastaAdi", hastaAdi)
                .getResultList();
